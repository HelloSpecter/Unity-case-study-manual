...
public class SteeringAgentComponent : MonoBehaviour 
{
	...
	void Update () 
	{
		...
		else if ( m_path != null )
		{
			...
			{
				newVelocity = ComputeArrivalVelocity( seekPos, destPos, currentFloorPosition, GetComponent<Rigidbody>().velocity );
			}
	...
	private Vector3 ComputeArrivalVelocity(Vector3 seekPos, Vector3 target, Vector3 position, Vector3 currentVelocity)
	{
		...
		Vector3 gravitationalForce = -Vector3.up * m_gravitationalAccelerationRate * GetComponent<Rigidbody>().mass;
